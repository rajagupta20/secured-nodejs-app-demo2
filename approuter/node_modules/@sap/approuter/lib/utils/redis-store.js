...
class RedisStore {
  ...
  async getExternalSession(id, req) {
    if (this.checkClient()) {
      log.debug(`Getting external session with id ${ id }`);
      try {
        const session = await this.client.get(redisKey(id, EXTERNAL_SESSION_PREFIX));
  ...
  async updateExternalSession(session) {
    if (this.checkClient()) {
      try {
        ...
        await this.client.set(redisKey(sessionToSave.externalSessionId, EXTERNAL_SESSION_PREFIX), compress(sessionToSave), 'PX', (sessionToSave.externalSessionExpiration * 1000) - Date.now());
      } catch (err) {
  ...
  async destroyExternalSession(id) {
    if (this.checkClient()) {
      try {
        log.debug(`Destroying external session with id ${ id }`);
        await this.client.del(redisKey(id, EXTERNAL_SESSION_PREFIX));
      } catch (err) {
